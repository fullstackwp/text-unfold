...
final class FSWP_ELT_text_unfold_addon
{
    ...
    public function is_compatible()
    {
        ...
        if( version_compare( ELEMENTOR_VERSION, self::MINIMUM_ELEMENTOR_VERSION, '<' ) ) {
            add_action( 'admin_notices', function() {?>
                <div class="notice notice-error is-dismissible">
                    <p><?php echo sprintf( esc_html( '%s is not compatible with your version of Elementor. Minimum required version is %s.', 'text-unfold' ), '<strong>'. esc_html( 'Text Unfold For Elementor', 'text-unfold' ).'</strong>', esc_html( self::MINIMUM_ELEMENTOR_VERSION, 'text-unfold' ) ); ?></p>
                </div>
            <?php });
            return false;
        }
        ...
        if( version_compare( PHP_VERSION, self::MINIMUM_PHP_VERSION, '<' ) ) {
            add_action( 'admin_notices', function() {?>
                <div class="notice notice-error is-dismissible">
                    <p><?php echo sprintf( esc_html( '%s is not compatible with your version of PHP. Minimum required version is %s.', 'text-unfold' ), '<strong>'. esc_html( 'Text Unfold For Elementor', 'text-unfold' ).'</strong>', esc_html( self::MINIMUM_PHP_VERSION, 'text-unfold' ) ); ?></p>  
                </div>
            <?php });
            return false;
        }
        ...
        if( version_compare( get_bloginfo('version' ), self::MINIMUM_WP_VERSION, '<' ) ) {
            add_action( 'admin_notices', function() {?>
                <div class="notice notice-error is-dismissible">
                    <p><?php echo sprintf( esc_html( '%s is not compatible with your version of WordPress. Minimum required version is %s.', 'text-unfold' ), '<strong>'. esc_html( 'Text Unfold For Elementor', 'text-unfold' ).'</strong>', esc_html( self::MINIMUM_WP_VERSION, 'text-unfold' ) ); ?></p> 
                </div>
            <?php });
            return false;
        }
